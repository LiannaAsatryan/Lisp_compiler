...
std::string correct_str(std::string str) {
        if(str[0] == '\"') {
                str.erase(0, 1);
...
void set_interpreter(token obj)
{
        ...
        if (args[2].get_type() == identifier) {
		std::string value = find_variable_value(correct_str(args[2].get_value())); 
		...
		change_variable_value(args[1].get_value(), correct_str(args[2].get_value())); 
	}
}
...
void execute_obj(token &obj, token arg)
{
		...
		if(::in_function) {
			obj.set_value(correct_str(funcs[::function_index].find_val(arg.get_value())));
		} else {
        		obj.set_value(correct_str(find_value(arg.get_value())));
		}
        } else {
        	 obj.set_value(correct_str(arg.get_value()));
        }
}
...
void interprete(token &obj)
{
		...
		std::cout <<  "ERROR " << args[0].get_value() << std::endl;
